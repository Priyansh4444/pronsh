...
export function Ring() {
    ...
    const environment = useEnvironment({ files: "/aerodynamics_workshop_1k.hdr" });
    ...
    return (
        <group ref={groupRef} dispose={null} scale={10} position={[0, 0, -325.5]}>
            ...
            <mesh position={[0, 0, 8]} rotation={[Math.PI / 2.0, 0, 0]} scale={3} ref={ref} geometry={(nodes1.Torus as THREE.Mesh).geometry}>
            ...
            <group scale={10}>
                <group ref={hourHandRef}>
                    ...
                    <mesh geometry={(nodes2.Plane004_1 as THREE.Mesh).geometry} scale={1.5}>
                        <meshBasicMaterial color={"#FFD700"} envMap={environment} />
                    </mesh>
                </group>
                <group ref={minuteHandRef}>
                    <mesh castShadow receiveShadow geometry={(nodes3.Plane005 as THREE.Mesh).geometry} scale={1.9}>
                        <meshBasicMaterial color={"#FFD700"} envMap={environment} />
                    </mesh>
                    <mesh castShadow receiveShadow geometry={(nodes3.Plane005_1 as THREE.Mesh).geometry} scale={1.9}>
                        <meshBasicMaterial color={"#FFD700"}  envMap={environment} />
                    </mesh>
                </group>
                <group ref={secondHandRef}>
                    <mesh castShadow receiveShadow geometry={(nodes4.Plane005 as THREE.Mesh).geometry} scale={2}>
                        <meshBasicMaterial color={"#FFD700"}  envMap={environment} />
                    </mesh>
                    <mesh castShadow receiveShadow geometry={(nodes4.Circle006 as THREE.Mesh).geometry} scale={2}>
                        <meshBasicMaterial color={"#FFD700"}  envMap={environment} />
                    </mesh>
                </group>
            </group>
